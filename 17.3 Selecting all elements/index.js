
const myFirstDiv = document.querySelector("#myFirstDiv");
const mySecondDiv = document.querySelector("#mySecondDiv");
const myThridDiv = document.querySelector("#myThridDiv");
const myFourthDiv = document.querySelector("#myFourthDiv");

myFirstDiv.style.color = '#a9dfbf';
mySecondDiv.style.color = '#d7bde2';
myThridDiv.style.color = '#85c1e9';
myFourthDiv.style.color = '#f7dc6f';

const divs = document.querySelectorAll('div');
console.log(divs);
divs[0].style.fontSize = '25px';